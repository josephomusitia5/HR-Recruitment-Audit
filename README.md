...
* **Data Transformation:** Cleaned, structured, and prepped raw HR datasets using **Power Query**.
* **Relational Architecture:** Built a robust **Data Model** to link recruitment activity logs with internal compensation limits.
* **Advanced DAX:** Engineered custom measures utilizing `CALCULATE`, `ALL`, and `AVERAGE` for dynamic data filtering.
* **Data Visualization:** Designed an interactive, slicer-driven dashboard minimizing visual clutter to emphasize actionable Key Performance Indicators (KPIs).
